...
class HandlerService:
    ...
    def save_handler_from_json(self, *, handler_data: str) -> Handler:
        serializer = HandlerSerializer(data=handler_data)
        if serializer.is_valid():
            return self.create_or_update_handler(
                handler_data=serializer.validated_data)

    @transaction.atomic
    def create_or_update_handler(self, *, handler_data: dict) -> Handler:
    ...
    def get_or_retrieve_handler(self, site_id: str) -> Handler:
        if Handler.objects.filter(epa_id=site_id).exists():
            ...
            if response.response.ok:
                return self.save_handler_from_json(
