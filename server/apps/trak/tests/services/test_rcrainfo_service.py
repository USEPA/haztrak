...
class TestRcrainfoService:
    ...
    def test_class_inits(self, testuser1):
        rcrainfo = RcrainfoService(username=testuser1.username, rcrainfo_env='preprod')
    ...
    def test_auto_authorized(self, test_user_profile, testuser1):
        rcrainfo = RcrainfoService(username=testuser1.username)
    ...
    def test_gets_credentials_correctly(self, testuser1, test_user_profile):
        ...
        rcrainfo = RcrainfoService(username=testuser1.username)
        auth_url = f'{rcrainfo.base_url}api/v1/auth/{test_user_profile.rcra_api_id}/{test_user_profile.rcra_api_key}'
        mock_token = "thisIsAMockToken"
        mock_token_exp = datetime.utcnow().replace(tzinfo=timezone.utc) + timedelta(
            minutes=20)
        ...
        with responses.RequestsMock() as mock:
            mock.add(responses.GET, auth_url,
                     json={
                         "token": mock_token,
                         "expiration": mock_token_exp.strftime(
                             rcrainfo.expiration_format)
                     },
                     status=200)
            # Make the request
