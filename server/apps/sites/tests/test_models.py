...
class TestRcraProfileModel:
    ...
    @pytest.fixture(autouse=True)
    def _setup_profile(self, epa_profile_factory):
        self.profile = epa_profile_factory()
    ...
    def test_is_api_user_returns_false_one_empty(self, epa_profile_factory, user_factory):
        profile = epa_profile_factory(rcra_api_id=None, user=user_factory(username="foobar"))
        assert not profile.is_api_user

    def test_is_api_user_false_when_empty(self, epa_profile_factory, user_factory):
